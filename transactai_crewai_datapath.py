...
import random
import os
from customtools import summary_tool_datapath_crewai, decimalremove_sum_tool, duckduckgo_search, internet_search_tool, add_tool,minus_tool,multiple_tool,division_tool, mathpower_tool,squareroot_tool
from dotenv import load_dotenv
import llms 
import json
from data.prepare_data import data_prepared
from mem0 import MemoryClient
# from llm_aws import bedrockllm
load_dotenv()

#select preferred large language model
# llm = llms.nvidia_hosted_llm  
llm = llms.groq_hosted_llm
# llm = llms.bedrock_llm
# llm = llms.llm_cohere #need to fix
...
datafilepath='data/data_generated100k.json'

# Get user inputs
# task_description = input("How can I help you today?\n")
task_description=f"""Based on this json data in this path {datafilepath}, please summarize total of transactionamount with mechbackitemcode 
and find the top 3 by total transactionamount."""
task_description2=f"""Based on this json data in this path {datafilepath}, please summarize the data and it's trends."""
...
transactions_assist = Agent(
    ...
    tools=[summary_tool_datapath_crewai],
    memory=True,
...
reviwer_assist = Agent(
    llm=llm,
    role='review assistant',
    goal='review the report generated by transactions assistant',
    backstory=dedent("""Instructions
                        You are a task reviewer. 
                        You expertise is review the summary report, make sure it matches to the template.
                        Do your best to produce the correct summary report. 
                    """),
    allow_delegation=False,
    verbose=True,
...
transactions_analysis_task = Task(
    ...
    description=dedent(f"""You will be give this tasks with data, please use your best knowledge to get the right answer:
                            ...
                            Instructions
                            ------------
                        {task_description} This is the json data in this file path: {datafilepath}                
...
# Define the team
crew = Crew(
  agents=[transactions_assist, reviwer_assist],
